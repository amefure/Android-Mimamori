...
open class OneTouchHelperCallback(private val recyclerView: RecyclerView): ItemTouchHelper.Callback() {
    ...
    // タップイベントに適合させる
    interface TapAdapter {
        fun onItemTap(adapterPosition: Int)
    }

    // 他のメソッドは省略

    private fun handleTapEvent(event: MotionEvent) {
        val childView = recyclerView.findChildViewUnder(event.x, event.y) ?: return
        val adapterPosition = recyclerView.getChildAdapterPosition(childView)
        val tapAdapter = recyclerView.adapter as? TapAdapter ?: return
        tapAdapter.onItemTap(adapterPosition)
    }

    // onCreate などで呼び出し、RecyclerViewにタップイベントを設定する
    fun setupTapEvent() {
